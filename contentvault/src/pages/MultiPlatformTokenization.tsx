...
import { createASAWithPera } from '../services/peraWalletService'
...
const MultiPlatformTokenization: React.FC = () => {
  ...
  const executeTokenization = async (tokenName: string, tokenSymbol: string, totalSupply: number) => {
    if (!scrapedContent || !address || !peraWallet) return
    ...
    try {
      ...
      const { txId, assetId } = await createASAWithPera({
        sender: address,
          ...
          {loading && (
            <div className="mt-4 p-3 bg-blue-500/20 border border-blue-500/50 rounded-xl">
              <p className="text-blue-200 text-sm text-center">
                ⏳ Please wait... Signing transaction with Pera Wallet and creating your token on Algorand blockchain.
              </p>
            </div>
          )}
        </form>
