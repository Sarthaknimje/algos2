...
export function getUserFriendlyMessage(error: Error | AppError | unknown): string {
  ...
  if (error instanceof Error) {
    ...
    // Wallet errors
    if (errorMessage.includes('wallet') || errorMessage.includes('pera')) {
      return 'Wallet connection error. Please ensure Pera Wallet is installed and unlocked.'
    }

    // Algorand/Blockchain errors
    if (errorMessage.includes('algorand') || errorMessage.includes('transaction')) {
...
export function getErrorType(error: Error | unknown): ErrorType {
  ...
  if (message.includes('wallet') || message.includes('pera')) {
    return ErrorType.WALLET
  }
  if (message.includes('auth') || message.includes('unauthorized')) {
    return ErrorType.AUTHENTICATION
  }
  if (message.includes('algorand') || message.includes('transaction') || message.includes('asa')) {
