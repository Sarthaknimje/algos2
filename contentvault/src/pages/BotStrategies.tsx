...
const PREBUILT_STRATEGIES: Array<{
...
}> = [
  {
    ...
    actionTemplate: 'buy {amount} ALGO of {symbol} when likes_1h_change > 20',
  ...
  },
  {
    ...
    actionTemplate: 'buy {amount} ALGO of {symbol} when views_1h_change > 30',
  ...
  },
  {
    ...
    actionTemplate: 'buy {amount} ALGO of {symbol} when comments_1h > 50',
        ...
        {isTokenSelectOpen && selectedStrategy && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70">
            ...
            >
              ...
              <div className="mb-6">
                <label className="block text-sm font-semibold text-white mb-2">Allocation (ALGO)</label>
                <input
                  type="number"
                  min={0.1}
                  step={0.1}
                      ...
                      >
                        <div className="flex items-center justify-between">
                          ...
                          <div className="text-right">
                            <p className="text-sm font-semibold text-gray-200">{token.current_price.toFixed(4)} ALGO</p>
                            {selectedToken?.asa_id === token.asa_id && (
              ...
              <div className="flex items-center justify-between">
                <div className="text-xs text-gray-400">
                  {selectedToken ? (
                    <span>
                      Strategy will run on <span className="text-cyan-300 font-semibold">{selectedToken.token_symbol}</span> with {allocation} ALGO allocation
                    </span>
                  ) : (
                ...
                <button
                  ...
                  onClick={async () => {
                    ...
                    try {
                      ...
                      alert(`✅ Strategy "${selectedStrategy.label}" is now active for ${selectedToken.token_symbol} with ${allocation} ALGO allocation!`)
                      setIsTokenSelectOpen(false)
                      setSelectedStrategy(null)
                      setSelectedToken(null)
                    } catch (error: any) {
                    ...
                    <div className="grid grid-cols-3 gap-4 mb-6">
                      ...
                      <div className="p-4 rounded-2xl bg-white/5 border border-white/10">
                        ...
                        <p className={`text-2xl font-bold ${(strategyExecutions.total_pnl || 0) >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}>
                          ...
                          {(strategyExecutions.total_pnl || 0).toFixed(4)} ALGO
                        </p>
                      </div>
                      <div className="p-4 rounded-2xl bg-white/5 border border-white/10">
                          ...
                          <tbody>
                            {strategyExecutions.executions.map((exec: any) => (
                              <tr key={exec.id} className="border-t border-white/5 hover:bg-white/5">
                                ...
                                <td className="px-4 py-3 text-right text-gray-200">{exec.price.toFixed(6)} ALGO</td>
                                <td className="px-4 py-3 text-right text-gray-200">{exec.total_value.toFixed(4)} ALGO</td>
                                <td className={`px-4 py-3 text-right font-semibold ${
                                  exec.pnl >= 0 ? 'text-emerald-400' : 'text-rose-400'
                                }`}>
                                  {exec.pnl >= 0 ? '+' : ''}{exec.pnl.toFixed(4)} ALGO
                                </td>
                              </tr>
                            ))}
                          </tbody>
